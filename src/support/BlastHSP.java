...
public class BlastHSP implements Comparable<BlastHSP> {
    ...
    */
	...
	public String getBedHit(){
		
		String color = "255,0,0";//red
		if( this.getIdentityPercentage() > 80){
			color = "255,128,0";//orange
			if( this.getIdentityPercentage() > 90){
				color = "255,255,0";//yellow
				if(this.getIdentityPercentage() > 95){
					color = "156,255,0";//light green
					if(this.getIdentityPercentage()>98){
						color = "0,255,0";//green
					}
				}
			}
		...
		return (this.getQueryName() + "\t" + this.getQueryStart() + "\t" + this.getQueryEnd() + "\t"+this.getHitName().split("_")[0]+"\t"+this.getIdentityPercentage()+"\t" + this.getHitStrand() + "\t" + this.getQueryStart() + "\t" + this.getQueryEnd() +"\t" + color );
		
		
	}
public String getBedHit_HitAsReference(){
		...
		return (this.getHitName() + "\t" + Math.min(this.getHitStart(), this.getHitEnd()) + "\t" + Math.max(this.getHitStart(), this.getHitEnd()) + "\t"+this.getQueryName()+"\t"+this.getIdentityPercentage()+"\t" + this.getHitStrand() + "\t" + Math.min(this.getHitStart(), this.getHitEnd()) + "\t" + Math.max(this.getHitStart(), this.getHitEnd()) +"\t" + color );
	...
	public String getSAM(){
		...
		int flag = 64; //read is first in pair
		if( this.getQueryStrand() == -1){
			flag = flag + 16;
		}
		sam = sam + "\t" + flag;
		
		String rname = this.getHitName();
		int pos = this.getHitStart();
		
		
		sam = sam + "\t" + rname + "\t" + pos +"\t255" ;   //mapq is set to 255 as in N/A
		
		String cigar= "";
		int numclip = this.getQueryStart()-1;
		if(numclip > 0){
			cigar = cigar + numclip + "H";
		}		
		char[] a = this.getQueryString().toCharArray();
		char[] b = this.getHitString().toCharArray();
		...
		for( int i = 0; i< a.length; i++){
			...
			if( myOperation.equalsIgnoreCase(operation)){
				count++;
			}else{
				cigar = cigar + count + operation;
				operation = myOperation;
			}
			 numclip = this.getQueryLength()-this.getQueryEnd();
			if(numclip > 0){
				cigar = cigar + numclip + "H";
			}
		...
		BioSequence seq = new BioSequence("",this.getQueryString().replaceAll("-", "") );
		seq.setQuality(30);
		
		sam = sam + "\t" + seq.getSequence() + "\t" + seq.getQuality(33);
