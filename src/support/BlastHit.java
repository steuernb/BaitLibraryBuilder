...
public class BlastHit implements Comparable<BlastHit> {
	...
	*/
	...
	public void removeHSP(double identityThreshold, int hspLength){
		Vector<BlastHSP> v = new Vector<BlastHSP>();
		for(Enumeration<BlastHSP> myenum = this.getHSPs().elements(); myenum.hasMoreElements();){
			BlastHSP hsp = myenum.nextElement();
			if(hsp.getIdentityPercentage()>=identityThreshold && hsp.getHspLength()>=hspLength){
				v.add(hsp);
			}
		}
		this.hsps = v;
	...
	public Vector<BlastHSP> getBestHSPCombination(){
		Vector<BlastHSP> v = new Vector<BlastHSP>();
		int[] queryCoverage = new int[this.getQueryLength()];
		int[] hitCoverage = new int[this.getHitLength()];
		
		int strand = this.getHSPs().get(0).getHitStrand();
		
		for(Enumeration<BlastHSP> myenum = this.getHSPs().elements(); myenum.hasMoreElements();){
			BlastHSP hsp = myenum.nextElement();
			
			if(strand != hsp.getHitStrand()){
				continue;
			}
			...
			for( int i = hsp.getQueryStart()-1; i< hsp.getQueryEnd(); i++){
				if(queryCoverage[i]>0){
					queryCovered++;
				}
			}
			
			for( int i = hsp.getHitStart()-1; i< hsp.getHitEnd(); i++){
				if(hitCoverage[i] >0){
					hitCovered++;
				}
			}
			
			if( hitCovered < 6  && queryCovered<6){
				v.add(hsp);
				for( int i = hsp.getQueryStart()-1; i< hsp.getQueryEnd(); i++){
					queryCoverage[i]++;
				}
