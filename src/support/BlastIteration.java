...
public class BlastIteration {
	...
	/**
	 * returns the best best HSP of the best hit.
	 */
	public BlastHSP getFirstHSP(){
		return this.getHits().get(0).getHSPs().get(0);
	}
	...
	public void removeHSP(double identityThreshold, int hspLength){
		...
		for(Enumeration<BlastHit> myenum1 = this.getHits().elements(); myenum1.hasMoreElements();){
			BlastHit b = myenum1.nextElement();
			b.removeHSP(identityThreshold, hspLength);
			if(b.getHSPs().size()>0){
				newHits.add(b);
			}
		}
	...
	public void removeDescriptions(){
		...
		for(Enumeration<BlastHit> myenum1 = this.getHits().elements(); myenum1.hasMoreElements();){
			...
			for(Enumeration<BlastHSP> myenum2 = b.getHSPs().elements(); myenum2.hasMoreElements();){
				...
				hsp.setQueryDescription("");
				hsp.setHitDescription("");
			}
		}
